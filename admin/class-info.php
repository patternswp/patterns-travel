 ...
 * @subpackage Patterns_Travel/Patterns_Travel_Info
 */
...
class Patterns_Travel_Info {
	...
	public function add_theme_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Theme Info', 'patterns-travel' ), esc_html__( 'Theme Info', 'patterns-travel' ), 'edit_theme_options', PATTERNS_TRAVEL_THEME_NAME, array( $this, 'info_screen' ) );
	}
	...
	public function enqueue_resources( $hook_suffix ) {
		...
		$unique_id = PATTERNS_TRAVEL_THEME_NAME . '-info';
		...
		$deps_file = PATTERNS_TRAVEL_PATH . 'build/admin/info/info.asset.php';
		...
		wp_enqueue_script( $unique_id, PATTERNS_TRAVEL_URL . 'build/admin/info/info.js', $dependency, $version, true );

		wp_enqueue_style( $unique_id, PATTERNS_TRAVEL_URL . 'build/admin/info/info.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'patterns_travel_info_localize',
			array(
				'version'  => $version,
...
/**
 * Return instance of Patterns_Travel_Info class
 *
 * @since 1.0.0
 *
 * @return Patterns_Travel_Info
 */
function patterns_travel_info() { //phpcs:ignore
	return Patterns_Travel_Info::instance();
}
patterns_travel_info()->run();
